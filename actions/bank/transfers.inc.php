...
$stmt = DB::run("SELECT * from bank_transfer WHERE BT_from = ? OR BT_to = ? ORDER BY BT_date DESC", [$session_id, $session_id]);

if (!$stmt->fetch(PDO::FETCH_LAZY)) {
    echo '<div class="text-info df mb-2 jcc">Du har ingen overførsler</div>';
} else {
?>
